// token-speed-dashboard · 历史记录持久化 store（纯逻辑，无 IO；文件读写由 server 侧薄层负责）
...
export function createStore(opts = {}) {
  ...
  function trend(now) {
    return { hour: trendSeries(now, HOUR, 48, floorHour), day: trendSeries(now, DAY, 7, floorDay) };
  }

  function trendSeries(now, step, count, floor) {
  ...
  function cache() {
    let sumInput = 0, sumRead = 0, sumWrite = 0;
    ...
    return {
      ...
      avgInput: byId.size > 0 ? Math.round(sumInput / byId.size) : null,
    };
  }
