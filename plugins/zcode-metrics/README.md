...
- 仪表盘（`http://127.0.0.1:<port>/`，端口见数据目录 `port.json`）显示：
  ...
  - **速度趋势**：近 24 小时（10 分钟粒度）/ 近 48 小时 / 近 7 天切换的吞吐加权折线，lite 后台任务汇总一列 + 窗口内请求数前 5 模型的单列曲线（v0.9.1 起不再绘制混合全模型的"主请求 avg"汇总线，单模型速度趋势更清晰）；图例即开关（点击隐藏/恢复单条曲线），悬停按时间列展示该时刻全部可见曲线读数；
