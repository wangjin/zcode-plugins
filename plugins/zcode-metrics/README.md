...
- 仪表盘（`http://127.0.0.1:<port>/`，端口见数据目录 `port.json`）显示：
  - **会话 tab 条**（多会话）："全部" + 各并行会话（短 ID + 状态点），实时区随选择切换，详见下节；
  ...
  - 趋势 / 模型对比 / 缓存分析三面板跟随 tab 口径："全部" = 全局，选中会话 = 仅该会话记录；
...
- **tab 语义**：顶部 tab 条 = "全部" + 各会话（短 ID 前 13 位 + 状态点：橙 = 生成中，绿 = 空闲）。"全部" 默认**自动跟随最近活跃会话**（实时区展示它）；点选某个会话后**锁定**，SSE 刷新不跳回；再点 "全部" 恢复自动跟随；被锁定的会话下线后自动回落。
...
- **过滤口径**：选中会话 tab 时，趋势 / 模型对比 / 缓存分析只统计该会话的记录（覆盖 7 天 history 与回填数据）；"全部"tab 维持全局口径。`history.jsonl` 不落 sessionId，重启恢复的记录无法归属会话，不参与过滤视图。
